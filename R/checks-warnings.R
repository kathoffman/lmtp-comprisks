...
check_missing_data <- function(data, trt, outcome, time_vary, baseline, cens, tau) {
  for (t in 1:tau) {
    ...
    if (any(is.na(as.matrix(data[ci & !di, c(check_trt_length(trt, time_vary, cens, tau)[t], unique(unlist(baseline)), unique(unlist(time_vary[t])))])))) {
...
check_for_variables <- function(data, trt, outcome, baseline, nodes, cens) {
  vars <- c(trt, outcome, unique(unlist(baseline)), unique(unlist(nodes)), cens)
...
check_trt_length <- function(trt, time_vary = NULL, cens = NULL, tau) {
  if (length(trt) == tau) {
    ...
    if (!is.null(time_vary) & is.null(cens)) {
      if (tau == length(time_vary$trt) && tau == length(time_vary$outcome)) {
        set_lmtp_options("trt", "standard")
        return(trt)
      }
...
check_factors <- function(data, trt, baseline, nodes) {
  vars <- c(trt, unique(unlist(baseline)), unique(unlist(nodes)))
